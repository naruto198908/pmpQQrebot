...
//	 public static String pmpGroupId = "100740797";// 测试群
	...
	class SendQuestionThread extends Thread {
		...
		private int questionWaitTime = 50;
		private int answerWaitTime = 15;

		private Map<String, String> curAnswer = new ConcurrentHashMap<String, String>();
		...
		public void addAnswer(String realQQ, String answer) {
			if (curQuestion != null) {
				curAnswer.put(realQQ, answer);
			}
		}

		public void run() {
			// 启动循环读取消息的心跳线程
			// Poll2Thread pt = new Poll2Thread( );
			...
			while (true) {
				try {
					...
					if (allQuestion.size() == 0) {
						allQuestion.addAll(temp);
						temp.clear();
					}
					int index = random.nextInt(allQuestion.size());
					curQuestion = allQuestion.get(index);
					curAnswer.clear();
					// System.out.println("==发送题目:" + q.getId() + "\n" +
					// q.getId() + "." + q.getQuestion());
					String res = sendMsg2Qun(pmpGroupUin, "\n" + curQuestion.getId() + ". " + curQuestion.getQuestion());
					// System.out.println("==发题响应消息:" + res);
					// 2.等30秒，收群消息，统计
					sleep(questionWaitTime * 1000);
					// 统计答案
					for (String realQQ : curAnswer.keySet()) {
						boolean isRight = false;
						if (curAnswer.get(realQQ).trim().equalsIgnoreCase(curQuestion.getAnswer().trim())) {
							isRight = true;
						}
						// 更新答题总次数
						// 更新用户成绩记录
						helper.updatePerformance(curQuestion.getId(), realQQ, isRight);
					}
					helper.addQuestionSendCount(curQuestion.getId());

					// 公布答案
					String analysis = ( curQuestion.getId() + ".    答案:" + curQuestion.getAnswer()
							+ "      解析:\n" + curQuestion.getAnalysis()).replace("<br>", "\n");
					...
					temp.add(curQuestion);
					// 等3秒
					sleep(answerWaitTime * 1000);
		...
		public void setQuestionWaitTime(int sec) {
			this.questionWaitTime = sec;
		}

		public void setAnswerWaitTime(int sec) {
			this.answerWaitTime = sec;
		}
	}
	...
	public class Poll2Thread extends Thread {
		...
		private SendQuestionThread sqThread;

		@Override
		public void run() {
			super.run();
			sqThread = new SendQuestionThread();
			sqThread.start();// 启动发题线程
			while (true) {
				try {
					...
					if (ob.getString("retcode").equals("0")) {
						...
						if (!flag.equals("message")) {
							if (flag.equals("group_message")) {
								...
								if (pmpGroupUin == null && pmpGroupId.equals(info_seq)) {
									pmpGroupUin = Long.parseLong(gid);
								}
								doReply(group_code, from_uin, content);
							}
						}
					}
				} catch (IOException e) {
		...
		// 回复消息处理线程
		private void doReply(String groupUin, String uin, String content) {
			//
			new QueryThread(Long.valueOf(groupUin), uin, content).start();
		}
		...
		class QueryThread extends Thread {
			...
			Pattern pKeword = Pattern.compile("#keyword:(\\S+).*");
			Pattern pTool = Pattern.compile("#tool:(\\S+).*");
			Pattern pITTO = Pattern.compile("#itto:(\\S+).*");
			Pattern pAnswer = Pattern.compile("\\s*([a-dA-D])*");
			...
			public QueryThread(long groupUin, String uin, String content) {
			...
			public void run() {
				...
				try {
					...
					try {
						...
						try {
							String res = sendMsg2Qun(pmpGroupUin, "打开数据库失败");
							// System.out.println("==打开数据库失败:" + res);
						} catch (IOException e) {
							e.printStackTrace();
						}
						return;
					...
					if ("#start".equalsIgnoreCase(content)) {
						sendMsg2Qun(pmpGroupUin, "get继续发题命令");
						sqThread._start();
					} else if ("#pause".equalsIgnoreCase(content)) {
						sendMsg2Qun(pmpGroupUin, "get暂停发题命令");
						sqThread._pause();
					} else if (content.startsWith("#set_question_waittime:")) {
						System.out.println("set_question_waittime");
						Pattern p = Pattern.compile("#set_question_waittime:(\\d+).*");
						Matcher m = p.matcher(content);
						int sec = -1;
						if (m.find()) {
							sec = Integer.valueOf(m.group(1));
						}
						if (sec > -1) {
							if (sec < 20) {
								sendMsg2Qun(pmpGroupUin, "问题等待时间必须大于20秒");
							} else if (sec > 150) {
								sendMsg2Qun(pmpGroupUin, "问题等待时间必须小于150秒");
							} else {
								sqThread.setQuestionWaitTime(sec);
								sendMsg2Qun(pmpGroupUin, "问题等待时间设置成功，下次生效.");
							}
						}
					} else if (content.startsWith("#set_answer_waittime:")) {
						System.out.println("set_answer_waittime");
						System.out.println("set_answer_waittime");
						Pattern p = Pattern.compile("#set_answer_waittime:(\\d+).*");
						Matcher m = p.matcher(content);
						int sec = -1;
						if (m.find()) {
							sec = Integer.valueOf(m.group(1));
						...
						if (sec > -1) {
							if (sec < 10) {
								sendMsg2Qun(pmpGroupUin, "答案等待时间必须大于10秒");
							} else if (sec > 60) {
								sendMsg2Qun(pmpGroupUin, "答案等待时间必须小于60秒");
							} else {
								sqThread.setAnswerWaitTime(sec);
								sendMsg2Qun(pmpGroupUin, "答案等待时间设置成功，下次生效.");
							}
						}
					} else if (content.startsWith("#itto:")) {
						// 查询数据库，然后输出
						Matcher m = pITTO.matcher(content);
						if (m.find()) {
							String processName = m.group(1);
							if (processName != null) {
								PmpProcess prc = helper.queryProcess(processName);
								String content = "无此过程名:" + processName;
								if (prc != null) {
									content = processName;
									content += "\n◆input:";
									for (int i = 0; i < prc.getInput().size(); i++) {
										content += (i + 1) + "." + prc.getInput().get(i).getName() + ";";
									}
									content += "\n◆tool&technology:";
									for (int i = 0; i < prc.getTool().size(); i++) {
										content += (i + 1) + "." + prc.getTool().get(i).getName() + ";";
									}
									content += "\n◆output:";
									for (int i = 0; i < prc.getOutput().size(); i++) {
										content += (i + 1) + "." + prc.getOutput().get(i).getName() + ";";
									}
								}
								sendMsg2Qun(pmpGroupUin, content);
							}
						}
					} else if (content.startsWith("#tool:")) {
						Matcher m = pTool.matcher(content);
						if (m.find()) {
							String str = m.group(1);
							if (str != null) {
								// 查询数据库，然后输出
								Tool t = helper.queryToolByName(str);
								// 如果没查到结果，推荐关键字
								String content = "无此工具技术:" + str;
								if (t != null) {
									content = t.getName() + ":" + t.getConception();
								} else {
									String s = helper.getToolNameQueryAdvice(str);
									if (s.length() > 0) {
					...
					} else if (content.startsWith("#myscore")) {
						// 一下是需要获取qq号的
						...
						User u = helper.queryUser(realQQ);
						String content = "暂未记录" + realQQ + "的成绩";
						if (u != null) {
							content = u.getUid() + "的成绩:答对" + u.getRightCount() + "个/共回答" + u.getAnswerCount() + "个";
							List<Integer> result = helper.queryWrongRecord(realQQ);
							if (result != null) {
								content += "\n答错题号:";
						...
						sendMsg2Qun(
								pmpGroupUin,
								"=========机器人使用说明==========\n1.开始发题.eg:#start\n2.暂停发题.eg:#pause\n3.查询工具技术名词解释.eg:#tool:分解\n4.查询过程输入、工具技术和输出:.eg:#itto:控制进度.\n5.回答正在进行的问题，成绩将会记录，在公布答案之前回答有效,允许修改答案.eg:B\n6.查询成绩.eg:#myscore");
					} else {
						String answer = null;
						...
						if (answer != null) {
							System.out.println(uin + "的答案:" + answer);
							// 一下是需要获取qq号的
							...
							sqThread.addAnswer(realQQ, answer);
						}
					}
				} catch (IOException e) {
					e.printStackTrace();
	...
	private void doWork() throws IOException, ScriptException, InterruptedException {
		...
		// 开始轮询
		Poll2Thread pt = new Poll2Thread();
		pt.ptwebqq = glovfwebqq;
		pt.psessionid = glopessionID;
		pt.start();
	...
	private String sendMsg2Qun(long group_uin, String content) throws IOException {
		System.out.println("==发送群消息:" + content);
		...
		try {
			...
			if (0 == result.optInt("retcode")) {
				...
				if (res != null && res.has("account")) {
					...
					uinMap.put(uin, String.valueOf(realQQ));
					return String.valueOf(realQQ);
				}
			}
		} catch (MalformedURLException e) {
			// TODO Auto-generated catch block
	...
	private void doXinTiao() {
		SendQuestionThread x = new SendQuestionThread();
